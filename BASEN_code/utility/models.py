...
# Load BASEN.json parameters
config_path = os.path.join(
    os.path.dirname(__file__),  # directory of models.py
    "..",  # go up to BASEN_code/
    "configs",  # enter configs/
    "BASEN.json"  # file name
)
...
if network_config["offline_0_online_1"] == 0:
    input_length = network_config["offline_feature_map_size"]
else:
    input_length = network_config["online_feature_map_size"]
...
class TCN(nn.Module):
    def __init__(self, input_dim, output_dim, BN_dim, hidden_dim,
        ...
        self.fusion = MultiLayerCrossAttention(input_size=input_length, layer=CMCA_layer_num, in_ch=BN_dim,
                                               kernel_size=kernel,
                                               dilation=1)
